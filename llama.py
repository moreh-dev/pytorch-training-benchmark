...
class LLaMAConfig:
    ...
    def estimate_flops_per_token(self, model, bsz):
        ...
        ffn1_flops = (
            2 * bsz * self.max_seq_len * self.hidden_dim * self.embedding_dim *
        ...
        qkv_flops = (2 * bsz * self.max_seq_len * self.embedding_dim *
                     (self.num_heads + 2 * self.num_kv_heads) * head_dim)
        attention_flops = 4 * bsz * self.max_seq_len**2 * self.num_heads * head_dim
        projection_flops = (2 * bsz * self.max_seq_len * self.embedding_dim *
        ...
        learnable_weight_tflops = (
            ((total_ffn_flops + qkv_flops + projection_flops) * self.num_layers
             + embedding_flops) * 3)
...
class Attention(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, num_kv_heads):
        ...
        self.in_proj = nn.Linear(embedding_dim,
                                 embedding_dim + 2 * self.kv_dim,
                                 bias=False)
        self.out_proj = nn.Linear(embedding_dim, embedding_dim, bias=False)
        self.use_sdpa = torch.cuda.is_available(
        ) and 'MI3' not in torch.cuda.get_device_name()
...
class RMSNorm(nn.Module):
    ...
    def forward(self, input):
        ...
        output = (
            input_float *
            torch.rsqrt(input_float.pow(2).mean(-1, keepdim=True) +
                        self.eps)).type_as(input) * self.weight
        return output


class LLaMABlock(nn.Module):

    def __init__(self, embedding_dim, hidden_dim, num_heads, num_kv_heads,
                 eps):
        super().__init__()
...
class LLaMA(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_layers,
        ...
        for i in range(num_layers):
            layers.append(
                LLaMABlock(embedding_dim, hidden_dim, num_heads, num_kv_heads,
                           eps))
...
class Fp8LLaMA(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_layers,
                 num_heads, num_kv_heads, max_seq_len, eps):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim).cpu()
        layers = []
        for i in range(num_layers):
            layers.append(
                Fp8LLaMABlock(embedding_dim, hidden_dim, num_heads,
                              num_kv_heads, eps).cpu())
        self.layers = nn.ModuleList(layers)
...
class Fp8LLaMABlock(te.TransformerLayer):
    ...
    def __init__(self, embedding_dim, hidden_dim, num_heads, num_kv_heads,
                 eps):
        super().__init__(hidden_size=embedding_dim,
